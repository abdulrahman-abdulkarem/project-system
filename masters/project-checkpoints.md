# PROJECT CHECKPOINTS — master source

**This file is the single source of truth for the on-demand checkpoints.**
...
**The shortcuts that load these** (defined in `project-rules.md`): `review`, `test check`, `schema check`, `lang check` / `rtl check` / `a11y check`, `perf pass`, `motion check`, `ship check`, `rules refresh`.

**A note on using them:** every checkpoint but one reports, it doesn't fix. Findings come back grouped by severity and nothing gets changed until you say which ones to act on. That's the same reason you review diffs rather than blind-accepting them — a tool that both finds and fixes gives you nothing to check its judgement against. `rules refresh` is the one exception: it writes, but only to its own delimited region, only after showing a diff, and only once you approve it.
