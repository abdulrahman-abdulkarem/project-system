...
**A note on using them:** a checkpoint reports, it doesn't fix. Findings come back grouped by severity and nothing gets changed until you say which ones to act on. That's the same reason you review diffs rather than blind-accepting them — a tool that both finds and fixes gives you nothing to check its judgement against.
