# RTL / BILINGUAL GUIDE — master source

**This file is the single source of truth for script- and direction-specific rules.**

The setup prompts write this content into an `RTL.md` at the project root, but **only when
the project's reading direction is RTL or bilingual**. An English-only project never sees it
and never pays for it.

That is the whole point of splitting it out. These rules are too detailed to sit in the
always-on block, and irrelevant to most projects — but on a bilingual project they apply from
the first component, not at review time. Keeping them in a file the project loads deliberately
lets them be thorough instead of abbreviated.

**When you change a rule:** edit it here, then run `python3 build-prompts.py`.

**What stays elsewhere:** universal accessibility lives in the language checkpoint's Part A and
applies to every project ever built, in any language. The two always-on direction rules
(logical properties, and directional-vs-semantic icons) stay in `project-rules.md`, because
they stop a mistake at the moment it is typed.

---

# ===================================================================
# GENERATED BLOCK BEGINS — copied verbatim into the setup prompts,
# which write it to RTL.md at the project root when the project is RTL or
# bilingual. Edit here, then run build-prompts.py.
# ===================================================================
